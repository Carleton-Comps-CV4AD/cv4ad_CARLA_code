...
def calculate_steering(target_location, vehicle):
    vehicle_location = vehicle.get_location()
    delta = target_location - vehicle_location
    angle = math.atan2(delta.y, delta.x)
    vehicle_rotation = vehicle.get_transform().rotation.yaw
    steering = angle - math.radians(vehicle_rotation)
    
    return steering % (2 * math.pi)  # Normalize to [0, 2*pi]
...
def main():
    ...
    try:
        ...
        route_1_indices = [70, 129, 28, 79, 101, 55, 57, 119, 58, 154, 147]
        route_1 = []
        ...
        camera2_bp.set_attribute('sensor_tick', str(1.0))
        ...
        camera.listen(lambda image: image.save_to_disk('_outRaw/raw%06d.png' % image.frame, cc))
        
        cc2 = carla.ColorConverter.CityScapesPalette
        camera2.listen(lambda image: image.save_to_disk('_outSeg/seg%06d.png' % image.frame, cc2))
